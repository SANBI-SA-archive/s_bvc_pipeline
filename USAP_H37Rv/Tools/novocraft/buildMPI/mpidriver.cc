...
void mpi_driver::run_master(int argc, char** argv)
{
    ...
    while(!eof()) {
        ...
        while(num_running > 0)
        {
            ...
            {
                if(sleep_for.tv_nsec < 100000000)
                    sleep_for.tv_nsec += sleep_for.tv_nsec / 10;
                yields++;
                nanosleep(&sleep_for, NULL);
                continue;
            }
            if(sleep_for.tv_nsec > 1000)
        ...
        #ifdef YIELD
            dT.stop();
//            fprintf(stderr, "# MPS %f YPS %f dt(uSec) %f\n", messages/dT.diff(), yields/dT.diff(), dT.diff()*1000000./messages);
        #endif
    }
    mpitracef("Barrier");
